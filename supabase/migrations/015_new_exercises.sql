...
SELECT
  ...
  '[{"name": "alpha=1.0 gives pure vector ranking","setup": "import math\ndocs = [\n  {\"id\": \"a\", \"embedding\": [1,0,0], \"terms\": [\"foo\"]},\n  {\"id\": \"b\", \"embedding\": [0,1,0], \"terms\": [\"bar\"]},\n  {\"id\": \"c\", \"embedding\": [0.9,0.1,0], \"terms\": [\"baz\"]}\n]\ndf = {\"foo\":1,\"bar\":1,\"baz\":1}\nquery_vec = [1,0,0]","call": "hybrid_search(query_vec, [], docs, df, 3, 2.0, alpha=1.0, top_k=2)","expected": "[\"a\", \"c\"]","visible": true},{"name": "alpha=0.0 gives pure BM25 ranking","setup": "import math\ndocs = [\n  {\"id\": \"a\", \"embedding\": [1,0,0], \"terms\": [\"rag\",\"rag\",\"rag\"]},\n  {\"id\": \"b\", \"embedding\": [0.9,0,0], \"terms\": [\"rag\"]},\n  {\"id\": \"c\", \"embedding\": [0.8,0,0], \"terms\": [\"other\"]}\n]\ndf = {\"rag\":2,\"other\":1}\nquery_terms = [\"rag\"]\nquery_vec = [1,0,0]","call": "hybrid_search(query_vec, query_terms, docs, df, 3, 1.67, alpha=0.0, top_k=2)[0]","expected": "\"a\"","visible": true},{"name": "zero BM25 scores falls back to vector only","setup": "import math\ndocs = [\n  {\"id\": \"a\", \"embedding\": [0.9,0.1], \"terms\": [\"foo\"]},\n  {\"id\": \"b\", \"embedding\": [0.1,0.9], \"terms\": [\"bar\"]}\n]\ndf = {}\nquery_vec = [1,0]\nquery_terms = [\"missing\"]","call": "hybrid_search(query_vec, query_terms, docs, df, 2, 1.0, alpha=0.5, top_k=1)","expected": "[\"a\"]","visible": true},{"name": "returns correct top_k count","setup": "import math\ndocs = [{\"id\": str(i), \"embedding\": [float(i),0], \"terms\": []} for i in range(10)]\ndf = {}\nquery_vec = [9,0]","call": "len(hybrid_search(query_vec, [], docs, df, 10, 1.0, alpha=1.0, top_k=3))","expected": "3","visible": true},{"name": "hybrid alpha=0.5 re-ranks by combined score (hidden)","setup": "import math\ndocs = [\n  {\"id\": \"keyword_winner\", \"embedding\": [0.5,0.5], \"terms\": [\"llm\",\"llm\",\"rag\",\"rag\",\"rag\"]},\n  {\"id\": \"vector_winner\",  \"embedding\": [1.0,0.0], \"terms\": [\"other\"]}\n]\ndf = {\"llm\":1,\"rag\":1,\"other\":1}\nquery_vec = [1,0]\nquery_terms = [\"rag\",\"llm\"]","call": "hybrid_search(query_vec, query_terms, docs, df, 2, 3.0, alpha=0.5, top_k=2)","expected": "[\"keyword_winner\", \"vector_winner\"]","visible": false}]'::jsonb,
  true,
  'medium',
  'rag',
  ...
  'code',
  'test_cases',
  '[{"level": 1,"text": "Think about what hybrid means here: you need two separate scores (vector cosine and BM25) for every document, then combine them. What does the combination formula look like when alpha=1.0 vs alpha=0.0?"},{"level": 2,"text": "Compute all BM25 scores first, then find the max. Dividing every score by the max normalises them to [0,1]. If max is 0, skip normalisation and use vector scores only."},{"level": 3,"text": "For each doc: vector_score = cosine_similarity(query_vec, doc[\"embedding\"]); raw_bm25 = bm25_score(...). Normalise: norm_bm25 = raw / max_bm25 (only if max > 0). Combined = alpha * vector + (1-alpha) * norm_bm25. Sort descending, return top-k IDs."}]'::jsonb,
...
SELECT
  ...
  '[{"name": "cache hit — same content returns cached embedding","setup": "import hashlib\ncache = {\"doc1\": {\"embedding\": [99.0, 99.0], \"hash\": hashlib.md5(\"hello\".encode()).hexdigest()}}","call": "get_or_compute_embedding(\"doc1\", \"hello\", cache)","expected": "[99.0, 99.0]","visible": true},{"name": "cache miss — new doc is stored","setup": "import hashlib\ncache = {}","call": "get_or_compute_embedding(\"new_doc\", \"test\", cache) and (\"new_doc\" in cache)","expected": "True","visible": true},{"name": "stale cache — content changed triggers recompute","setup": "import hashlib\nold_hash = hashlib.md5(\"old content\".encode()).hexdigest()\ncache = {\"doc1\": {\"embedding\": [42.0], \"hash\": old_hash}}","call": "result = get_or_compute_embedding(\"doc1\", \"new content\", cache); result == [42.0]","expected": "False","visible": true},{"name": "hash is stored after compute","setup": "import hashlib\ncache = {}","call": "get_or_compute_embedding(\"d\", \"abc\", cache); cache[\"d\"][\"hash\"] == hashlib.md5(\"abc\".encode()).hexdigest()","expected": "True","visible": true},{"name": "repeated call with same content uses cache (hidden)","setup": "import hashlib\ncache = {}\nget_or_compute_embedding(\"doc\", \"v2\", cache)\ne1 = get_or_compute_embedding(\"doc\", \"v2\", cache)","call": "e2 = get_or_compute_embedding(\"doc\", \"v2\", cache); e1 == e2","expected": "True","visible": false}]'::jsonb,
  true,
  'medium',
  'rag',
  ...
  'code',
  'test_cases',
  '[{"level": 1,"text": "The function checks if the doc_id is in the cache — but it never checks whether the cached embedding is still valid. What information would you need to store alongside the embedding to detect staleness?"},{"level": 2,"text": "A content hash (MD5 of the content string) stored alongside the embedding lets you detect changes. Use hashlib.md5(content.encode()).hexdigest(). If the stored hash does not equal the current hash, the cache is stale."},{"level": 3,"text": "current_hash = hashlib.md5(content.encode()).hexdigest(). Cache hit condition: doc_id in cache AND cache[doc_id][\"hash\"] == current_hash. On miss: compute embedding, store {\"embedding\": ..., \"hash\": current_hash}."}]'::jsonb,
...
SELECT
  ...
  '[{"name": "valid call returns (True, empty string)","setup": "schema = {\"name\": \"search\", \"properties\": {\"query\": {\"type\": \"string\"}, \"k\": {\"type\": \"integer\"}}, \"required\": [\"query\"]}","call": "validate_tool_call({\"name\": \"search\", \"arguments\": {\"query\": \"rag\", \"k\": 5}}, schema)","expected": "(True, \"\")","visible": true},{"name": "missing required field returns (False, ...)","setup": "schema = {\"name\": \"search\", \"properties\": {\"query\": {\"type\": \"string\"}}, \"required\": [\"query\"]}","call": "validate_tool_call({\"name\": \"search\", \"arguments\": {}}, schema)[0]","expected": "False","visible": true},{"name": "wrong argument type returns (False, ...)","setup": "schema = {\"name\": \"search\", \"properties\": {\"k\": {\"type\": \"integer\"}}, \"required\": [\"k\"]}","call": "validate_tool_call({\"name\": \"search\", \"arguments\": {\"k\": \"five\"}}, schema)[0]","expected": "False","visible": true},{"name": "unknown tool name returns (False, ...)","setup": "schema = {\"name\": \"search\", \"properties\": {}, \"required\": []}","call": "validate_tool_call({\"name\": \"wrong_tool\", \"arguments\": {}}, schema)[0]","expected": "False","visible": true},{"name": "optional fields may be absent — still valid (hidden)","setup": "schema = {\"name\": \"embed\", \"properties\": {\"text\": {\"type\": \"string\"}, \"model\": {\"type\": \"string\"}}, \"required\": [\"text\"]}","call": "validate_tool_call({\"name\": \"embed\", \"arguments\": {\"text\": \"hello\"}}, schema)","expected": "(True, \"\")","visible": false}]'::jsonb,
  true,
  'medium',
  'agent_orchestration',
  ...
  'code',
  'test_cases',
  '[{"level": 1,"text": "Read the required-field check carefully: what is it actually testing? It checks if each required field exists in the schema properties — but properties defines ALL possible fields. What should it be checking instead?"},{"level": 2,"text": "Bug 1: check if the field is in args (the caller-supplied arguments), not in properties. Bug 2: when isinstance() returns False (type mismatch), the code returns True. What should it return instead?"},{"level": 3,"text": "Fix 1: change `if field not in properties` to `if field not in args`. Fix 2: change the inner return to `return False, f\"Field {field} expected {exp_type}, got {type(value).__name__}\"`."}]'::jsonb,
...
SELECT
  ...
  '[{"name": "single result — handoff has 1 entry","setup": "h = AgentHandoff()\nh.record_result(\"search\", \"found 3 docs\")","call": "len(h.get_handoff_context()[\"tool_results\"])","expected": "1","visible": true},{"name": "multiple results — total_calls is correct","setup": "h = AgentHandoff()\nh.record_result(\"search\", \"docs\")\nh.record_result(\"embed\", \"vectors\")\nh.record_result(\"rerank\", \"ranked\")","call": "h.get_handoff_context()[\"total_calls\"]","expected": "3","visible": true},{"name": "all_succeeded False when any failure","setup": "h = AgentHandoff()\nh.record_result(\"search\", \"ok\")\nh.record_result(\"embed\", \"timeout\", success=False)","call": "h.get_handoff_context()[\"all_succeeded\"]","expected": "False","visible": true},{"name": "all_succeeded True when all succeed","setup": "h = AgentHandoff()\nh.record_result(\"a\", \"ok\")\nh.record_result(\"b\", \"ok\")","call": "h.get_handoff_context()[\"all_succeeded\"]","expected": "True","visible": true},{"name": "results preserved in insertion order (hidden)","setup": "h = AgentHandoff()\nfor i in range(5):\n    h.record_result(f\"tool_{i}\", f\"result_{i}\")","call": "[r[\"tool\"] for r in h.get_handoff_context()[\"tool_results\"]]","expected": "[\"tool_0\", \"tool_1\", \"tool_2\", \"tool_3\", \"tool_4\"]","visible": false}]'::jsonb,
  true,
  'easy',
  'agent_orchestration',
  ...
  'code',
  'test_cases',
  '[{"level": 1,"text": "Look at the record_result method. After calling it three times, how many items does tool_results contain? Trace through the code — what does assigning to self.tool_results do on each call?"},{"level": 2,"text": "self.tool_results = [{...}] creates a brand-new single-element list each time, replacing whatever was there before. Python list assignment replaces; you need the method that adds to the existing list."},{"level": 3,"text": "Replace self.tool_results = [{...}] with self.tool_results.append({...}). The .append() method mutates the existing list in place, preserving all previous entries."}]'::jsonb,
...
SELECT
  ...
  'open_ended',
  'rubric',
  '[{"name": "store_selection","description": "Identifies a durable, shared memory store (not in-process) and justifies why it solves the crash-loss and cross-agent visibility problems."},{"name": "staleness_handling","description": "Proposes a concrete mechanism for detecting and handling stale reads — e.g., versioning, ETags, read-your-writes, or change notifications."},{"name": "idempotency","description": "Addresses the Writer duplicate-output problem with a specific idempotency approach (e.g., output IDs, completion flags, or a write-once key store)."},{"name": "failure_mode_awareness","description": "Identifies at least one remaining failure mode (race condition, store unavailability, etc.) and proposes a mitigation rather than claiming the design is bulletproof."}]'::jsonb
...
SELECT
  ...
  '[{"name": "new key is appended","setup": "mem = [{\"key\": \"user_name\", \"value\": \"Alice\", \"timestamp\": 1.0}]","call": "result = update_agent_memory(mem, {\"key\": \"task\", \"value\": \"summarise\", \"timestamp\": 2.0}); len(result)","expected": "2","visible": true},{"name": "existing key is replaced not duplicated","setup": "mem = [{\"key\": \"status\", \"value\": \"pending\", \"timestamp\": 1.0}]","call": "[f[\"value\"] for f in update_agent_memory(mem, {\"key\": \"status\", \"value\": \"done\", \"timestamp\": 2.0}) if f[\"key\"] == \"status\"]","expected": "[\"done\"]","visible": true},{"name": "does not modify input list","setup": "mem = [{\"key\": \"a\", \"value\": \"1\", \"timestamp\": 0.0}]","call": "update_agent_memory(mem, {\"key\": \"a\", \"value\": \"2\", \"timestamp\": 1.0}); len(mem)","expected": "1","visible": true},{"name": "empty memory — fact is added","setup": "","call": "len(update_agent_memory([], {\"key\": \"goal\", \"value\": \"research\", \"timestamp\": 1.0}))","expected": "1","visible": true},{"name": "multiple updates to same key leave exactly one entry (hidden)","setup": "m = []\nm = update_agent_memory(m, {\"key\": \"ctx\", \"value\": \"v1\", \"timestamp\": 1.0})\nm = update_agent_memory(m, {\"key\": \"ctx\", \"value\": \"v2\", \"timestamp\": 2.0})\nm = update_agent_memory(m, {\"key\": \"ctx\", \"value\": \"v3\", \"timestamp\": 3.0})","call": "[f[\"value\"] for f in m if f[\"key\"] == \"ctx\"]","expected": "[\"v3\"]","visible": false}]'::jsonb,
  true,
  'easy',
  'agent_orchestration',
  ...
  'code',
  'test_cases',
  '[{"level": 1,"text": "An agent memory should never grow unboundedly with duplicate keys — if you learn status: done after status: pending, only the latest should survive. The current code just appends. What operation would let you replace an existing entry?"},{"level": 2,"text": "Filter out any existing fact with the same key before adding the new one. A list comprehension works well: [f for f in memory if f[\"key\"] != new_fact[\"key\"]]. Then concatenate the new fact."},{"level": 3,"text": "return [f for f in memory if f[\"key\"] != new_fact[\"key\"]] + [new_fact]. This filters all old entries with the same key and appends the new one at the end. The input list is never modified."}]'::jsonb,
...
SELECT
  ...
  '[{"name": "parses Score: N/10 format","setup": "","call": "parse_judge_score(\"Score: 7/10\")","expected": "7","visible": true},{"name": "parses give this a N format","setup": "","call": "parse_judge_score(\"I give this a 8 out of 10\")","expected": "8","visible": true},{"name": "parses bold rating format","setup": "","call": "parse_judge_score(\"**Rating: 6**\")","expected": "6","visible": true},{"name": "raises ValueError when no score found","setup": "caught = False\ntry:\n    parse_judge_score(\"This response is completely wrong.\")\nexcept ValueError:\n    caught = True","call": "caught","expected": "True","visible": true},{"name": "parses inline N/10 pattern (hidden)","setup": "","call": "parse_judge_score(\"The answer deserves 9/10 for accuracy.\")","expected": "9","visible": false}]'::jsonb,
  true,
  'medium',
  'evaluation_testing',
  ...
  'code',
  'test_cases',
  '[{"level": 1,"text": "LLM judges use many different phrasings: Score: 7/10, I give this a 8, **Rating: 6**. A single regex won''t catch all of them. How would you try multiple patterns in sequence?"},{"level": 2,"text": "Write separate regex patterns for each format. Use re.search() with re.IGNORECASE. Try patterns from most specific to most general. Return int(match.group(1)) on the first match."},{"level": 3,"text": "Patterns to try: r\"(?:score|rating)\\s*[:\\-]?\\s*(10|[1-9])(?:/10)?\" for Score/Rating formats; r\"give\\s+this\\s+a?\\s+(10|[1-9])\" for natural language; r\"\\b(10|[1-9])\\s*/\\s*10\\b\" for N/10 inline. Raise ValueError if none match."}]'::jsonb,
  $fs$import re
...
SELECT
  ...
  '[{"name": "blocks when metric degrades beyond threshold","setup": "","call": "should_block_deploy({\"accuracy\": 0.80}, {\"accuracy\": 0.74}, threshold=0.05)[0]","expected": "True","visible": true},{"name": "allows when degradation is within threshold","setup": "","call": "should_block_deploy({\"accuracy\": 0.80}, {\"accuracy\": 0.78}, threshold=0.05)[0]","expected": "False","visible": true},{"name": "never blocks on improvement","setup": "","call": "should_block_deploy({\"f1\": 0.70}, {\"f1\": 0.85}, threshold=0.05)[0]","expected": "False","visible": true},{"name": "returns name of the blocking metric","setup": "r = should_block_deploy({\"precision\": 0.90, \"recall\": 0.80}, {\"precision\": 0.89, \"recall\": 0.70}, threshold=0.05)","call": "r[1]","expected": "\"recall\"","visible": true},{"name": "only metric c degrades — returns c (hidden)","setup": "r = should_block_deploy({\"a\": 1.0, \"b\": 1.0, \"c\": 1.0}, {\"a\": 0.99, \"b\": 0.99, \"c\": 0.50}, threshold=0.05)","call": "r[0] and r[1] == \"c\"","expected": "True","visible": false}]'::jsonb,
  true,
  'medium',
  'evaluation_testing',
  ...
  'code',
  'test_cases',
  '[{"level": 1,"text": "Trace through the function with baseline={\"accuracy\": 0.80} and new={\"accuracy\": 0.74}. Should this block? What does the current code compute for change, and what does it compare against?"},{"level": 2,"text": "Bug 1: the change is computed as new - baseline (absolute), but the threshold is a relative percentage (0.05 = 5%). You need (baseline - new) / baseline to get relative degradation. Bug 2: check if degradation > threshold, not if improvement > threshold."},{"level": 3,"text": "relative_degradation = (base_val - new_val) / base_val. If relative_degradation > threshold: block. This is positive when new < baseline (worse), negative when new > baseline (better). Guard against base_val == 0 with a continue."}]'::jsonb,
...
SELECT
  ...
  '[{"name": "fully grounded answer has score 0.0","setup": "","call": "detect_hallucination(\"the cat sat on the mat\", [\"the cat sat on the mat\"], n=3)[\"hallucination_score\"]","expected": "0.0","visible": true},{"name": "unsupported answer has score > 0","setup": "score = detect_hallucination(\"Paris is the capital of Germany\", [\"Berlin is the capital of Germany\"], n=3)[\"hallucination_score\"]","call": "score > 0","expected": "True","visible": true},{"name": "empty answer returns all zeros","setup": "","call": "detect_hallucination(\"\", [\"some source document\"], n=3)","expected": "{\"unsupported_count\": 0, \"supported_count\": 0, \"hallucination_score\": 0.0}","visible": true},{"name": "counts add up to total ngrams","setup": "r = detect_hallucination(\"the model returns an answer\", [\"the model\"], n=3)","call": "r[\"supported_count\"] + r[\"unsupported_count\"] > 0","expected": "True","visible": true},{"name": "case-insensitive matching (hidden)","setup": "","call": "detect_hallucination(\"The RAG Pipeline\", [\"the rag pipeline is fast\"], n=2)[\"hallucination_score\"]","expected": "0.0","visible": false}]'::jsonb,
  true,
  'hard',
  'evaluation_testing',
  ...
  'code',
  'test_cases',
  '[{"level": 1,"text": "The function should check whether phrases from the answer appear in the source documents. Start by splitting the answer into words, then think about what size of phrases (n-grams) you want to check."},{"level": 2,"text": "Generate all n-grams of size 2 through n from the answer words. For each n-gram, check if it appears as a substring in the concatenated source text (lowercased). Count supported vs unsupported."},{"level": 3,"text": "words = answer.lower().split(). For size in range(2, n+1): for i in range(len(words)-size+1): ngrams.append(\" \".join(words[i:i+size])). sources_lower = \" \".join(s.lower() for s in sources). supported = sum(1 for ng in ngrams if ng in sources_lower)."}]'::jsonb,
...
SELECT
  ...
  'open_ended',
  'rubric',
  '[{"name": "gap_identification","description": "Correctly identifies that the eval set is a static historical snapshot and does not cover queries about recent changes — and connects this to why 91% offline accuracy is misleading."},{"name": "retrieval_diagnosis","description": "Explains why the retrieval layer is a root cause: stale embeddings mean new documents are not retrievable, so the generator is working from an incomplete index — not just generating bad answers from good context."},{"name": "concrete_eval_fixes","description": "Proposes two specific, actionable eval methodology changes — not vague suggestions. At minimum: a freshness-stratified eval cohort AND retrieval-quality metrics (recall@k, MRR) tested independently of generation quality."},{"name": "freshness_metric_design","description": "Describes a concrete data freshness metric that could be added to the CI gate — what it measures, how it is computed, and why a threshold on it would have caught this failure."}]'::jsonb
...
SELECT
  ...
  '[{"name": "child inherits parent trace_id","setup": "ctx = {\"trace_id\": \"trace-abc\", \"span_id\": \"span-001\", \"depth\": 0}","call": "create_child_span(\"retrieval\", ctx).trace_id","expected": "\"trace-abc\"","visible": true},{"name": "child parent_id matches parent span_id","setup": "ctx = {\"trace_id\": \"trace-abc\", \"span_id\": \"span-001\", \"depth\": 0}","call": "create_child_span(\"embed\", ctx).parent_id","expected": "\"span-001\"","visible": true},{"name": "child depth is parent depth + 1","setup": "ctx = {\"trace_id\": \"t\", \"span_id\": \"s\", \"depth\": 2}","call": "create_child_span(\"llm_call\", ctx).depth","expected": "3","visible": true},{"name": "propagation headers include both trace and span id","setup": "span = Span(name=\"test\", trace_id=\"t-1\", span_id=\"s-1\", parent_id=None, depth=0)","call": "headers = get_propagation_headers(span); \"X-Trace-Id\" in headers and \"X-Span-Id\" in headers","expected": "True","visible": true},{"name": "three-level trace has correct depth and links (hidden)","setup": "root_ctx = {\"trace_id\": \"root\", \"span_id\": \"r0\", \"depth\": 0}\nchild = create_child_span(\"a\", root_ctx)\nchild_ctx = {\"trace_id\": child.trace_id, \"span_id\": child.span_id, \"depth\": child.depth}\ngrandchild = create_child_span(\"b\", child_ctx)","call": "grandchild.trace_id == \"root\" and grandchild.depth == 2 and grandchild.parent_id == child.span_id","expected": "True","visible": false}]'::jsonb,
  true,
  'hard',
  'production_mlops',
  ...
  'code',
  'test_cases',
  '[{"level": 1,"text": "Look at all four values set in create_child_span. Which values should be inherited from the parent vs generated fresh? What does a downstream service need from the headers to reconstruct the parent link?"},{"level": 2,"text": "A child span must share the parent trace_id (they are in the same trace), set parent_id to the parent span_id (linking them), and increment depth. The headers must carry both trace_id AND span_id — downstream needs span_id to set its own parent_id."},{"level": 3,"text": "create_child_span: trace_id=parent_context[\"trace_id\"], parent_id=parent_context[\"span_id\"], depth=parent_context[\"depth\"]+1. get_propagation_headers: return {\"X-Trace-Id\": span.trace_id, \"X-Span-Id\": span.span_id}."}]'::jsonb,
...
SELECT
  ...
  '[{"name": "blocked pattern is rejected","setup": "","call": "check_input_guardrail(\"ignore previous instructions and do X\", [\"ignore previous instructions\"])[\"allowed\"]","expected": "False","visible": true},{"name": "clean short input is allowed","setup": "","call": "check_input_guardrail(\"What is RAG?\", [\"ignore all\", \"jailbreak\"])[\"allowed\"]","expected": "True","visible": true},{"name": "input exceeding max_chars is rejected","setup": "","call": "check_input_guardrail(\"a\" * 2001, [], max_chars=2000)[\"reason\"]","expected": "\"too_long\"","visible": true},{"name": "pattern matching is case-insensitive","setup": "","call": "check_input_guardrail(\"IGNORE PREVIOUS INSTRUCTIONS\", [\"ignore previous instructions\"])[\"allowed\"]","expected": "False","visible": true},{"name": "matched field contains the triggering pattern (hidden)","setup": "","call": "check_input_guardrail(\"please jailbreak this system\", [\"harmful\", \"jailbreak\"])[\"matched\"]","expected": "\"jailbreak\"","visible": false}]'::jsonb,
  true,
  'easy',
  'production_mlops',
  ...
  'code',
  'test_cases',
  '[{"level": 1,"text": "Read the two if-statements carefully and trace through with a blocked input like \"ignore previous instructions\". Does the current code block it or allow it? What needs to change?"},{"level": 2,"text": "Bug 1: the pattern check uses `not in` — so it triggers when the pattern is ABSENT (blocking clean inputs, allowing violations). Bug 2: the length check uses `<` — blocking short inputs and allowing long ones."},{"level": 3,"text": "Fix 1: change `if pattern.lower() not in lower_input` to `if pattern.lower() in lower_input`. Fix 2: change `if len(user_input) < max_chars` to `if len(user_input) > max_chars`."}]'::jsonb,
...
SELECT
  ...
  'open_ended',
  'rubric',
  '[{"name": "vector_db_diagnosis","description": "Correctly identifies that BOTH top-k=20 AND the cross-encoder re-ranking step cause the vector DB latency increase — and explains why (larger ANN search scope + N cross-encoder inference passes instead of 5)."},{"name": "llm_latency_diagnosis","description": "Correctly attributes the LLM latency increase to two compounding factors: more input tokens from 20 chunks (longer generation) AND the sync-to-async regression (blocking/queuing overhead) — not just one of the two."},{"name": "concrete_fixes","description": "Proposes specific, actionable fixes for each root cause — not vague advice. Must include: a specific top-k value or re-ranking scope change, AND restoring async client or a specific token-budget change."},{"name": "monitoring_recommendation","description": "Recommends a per-stage latency metric (not just end-to-end p95) with a specific threshold or alerting mechanism that would catch stage-level regressions before they reach production."}]'::jsonb
